...
function renderPagination() {
  ...
  if (currentPage > 1) html += `<button data-action="page" data-page="${currentPage - 1}">‹</button>`;
  for (let i = start; i <= end; i++) {
    if (i === currentPage) {
      html += `<span class="pagination__current">${i}</span>`;
    } else {
      html += `<button data-action="page" data-page="${i}">${i}</button>`;
    }
  }
  if (currentPage < tp) html += `<button data-action="page" data-page="${currentPage + 1}">›</button>`;
  html += `<span class="pagination__counter">${PAGE_SIZE * (currentPage - 1) + 1}–${Math.min(PAGE_SIZE * currentPage, filteredRepos.length)} / ${filteredRepos.length}</span>`;
  html += '</div>';
  return html;
}
...
function renderPage() {
  ...
  if (total === 0) {
    container.setHTML(`<div class="empty-state">
      <div class="empty-state__cta">リポジトリが見つかりません</div>
  ...
  const searchBar = `<div class="search-bar">
    <input type="search" id="search-input" class="policy-select search-bar__input" placeholder="リポジトリを検索…" aria-label="リポジトリを検索" value="${esc(prevQ)}">
  </div>`;
